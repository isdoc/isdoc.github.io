...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
   ...
   <sch:pattern>
      <sch:title>Jednotky jednotlivých šarží v jedné řádce musí být stejné a odpovídat jednotce množství pro celou řádku</sch:title>
      <sch:rule context="isdoc:Invoice/isdoc:InvoiceLines/isdoc:InvoiceLine[isdoc:Item/isdoc:StoreBatches]">
            <sch:assert test="count(distinct-values(isdoc:Item/isdoc:StoreBatches/isdoc:StoreBatch/isdoc:Quantity/@unitCode)) le 1">Jednotka v rozpisu všech šarží/sériových čísel (element
          StoreBatches) musí být stejná jako jednotka pro množství na řádce faktury.
          ...
          jako množství na řádce faktury. </sch:assert>            
            <sch:assert test="if (isdoc:InvoicedQuantity/@unitCode) then                                 every $q in isdoc:Item/isdoc:StoreBatches/isdoc:StoreBatch/isdoc:Quantity) satisfies $q/@unitCode = isdoc:InvoicedQuantity/@unitCode                               else true()">Jednotka v rozpisu všech šarží/sériových čísel (element
          StoreBatches) musí být stejná jako jednotka pro množství na řádce faktury.
          Jednotky u šarží jedné řádky faktury musí být stejné.
          Pokud atribut pro jednotku v rozpisu šarží/sériových čísel není uveden, 
          tak se předpokládá, že je množství uvedeno ve stejné jednotce
   ...
   <sch:pattern>
      ...
      <sch:rule context="isdoc:Invoice/isdoc:InvoiceLines/isdoc:InvoiceLine[isdoc:Item/isdoc:StoreBatches]">
            <sch:assert test="sum(isdoc:Item/isdoc:StoreBatches/isdoc:StoreBatch/isdoc:Quantity) = isdoc:InvoicedQuantity">Součet Quantity ze všech záznamů rozpisu šarží/sériových čísel musí odpovídat InvoicedQuantity dané řádky faktury.</sch:assert>            
          </sch:rule>
   </sch:pattern>
   <sch:pattern>
      <sch:title>Sekundární identifikace zboží může být uvedena pouze v případě, že je uvedena i primární</sch:title>
      <sch:rule context="isdoc:Invoice/isdoc:InvoiceLines/isdoc:InvoiceLine/isdoc:Item[isdoc:SecondarySellersItemIdentification]">
            <sch:assert test="isdoc:SellersItemIdentification">Nepovinnou položku SecondarySellersItemIdentification lze uvést pouze v případě, že je uvedena také položka SellersItemIdentification.</sch:assert>            
          </sch:rule>
   </sch:pattern>
   <sch:pattern>
      <sch:title>Terciální identifikace zboží může být uvedena pouze v případě, že je uvedena i primární a sekundární</sch:title>
      <sch:rule context="isdoc:Invoice/isdoc:InvoiceLines/isdoc:InvoiceLine/isdoc:Item[isdoc:TertiarySellersItemIdentification]">
            <sch:assert test="isdoc:SellersItemIdentification and isdoc:SecondarySellersItemIdentification">Nepovinnou položku TertiarySellersItemIdentification lze uvést pouze v případě, že jsou uvedeny také položky SellersItemIdentification a SecondarySellersItemIdentification.</sch:assert>            
          </sch:rule>
   </sch:pattern>
</sch:schema>